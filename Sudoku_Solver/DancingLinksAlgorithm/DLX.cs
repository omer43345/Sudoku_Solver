    ...
    public class DLX
    {
        ...
        private void CreateRowNodes(List<DancingLinksColumnNode> columnNodes, byte[,] coverMatrix) {
            for(int row=0; row<_coverMatrixRowCount ; row++) {
                ...
                for(int column=0;column< _coverMatrixColCount;column++) {
                    if(coverMatrix[row,column]==1) {// If in the cover matrix, there is a 1 in this position then add a node to the column
        ...
        private DancingLinksColumnNode SelectBestColumn() {
            ...
            for(DancingLinksColumnNode column=(DancingLinksColumnNode)_root.Right;column!=_root;column=(DancingLinksColumnNode)column.Right) {
